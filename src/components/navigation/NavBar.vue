<template>
  <nav>
    ...
    <v-navigation-drawer app v-if='this.$store.getters.isAuthenticated' clipped left v-model='drawer' color='#1c1c1c' width='185px'>
      <v-list dense>
        <v-list-item v-for='item in items' :key='item.title' :to='item.link' link>
          <v-icon color='teal'>{{item.icon}}</v-icon>
          <v-list-item>
            <label style='color:#dedede'>{{item.title}}</label>
          </v-list-item>
        </v-list-item>
      </v-list>
    </v-navigation-drawer>
